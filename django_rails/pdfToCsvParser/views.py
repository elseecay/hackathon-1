...
def get_page_km(page_img) -> int:
    line = page_img[2020:2100, 1490:1515]
    line = cv2.rotate(line, cv2.ROTATE_90_COUNTERCLOCKWISE)
    chars = line_to_symbols(line)
    try:
        return int(''.join(chars))
...
def send_pdf(req):
	res = {}
	if req.method == 'POST':
		pdf = req.POST.get('file')
		base64string = pdf[28:]
		with open("test.pdf", "wb") as f:
			f.write(base64.decodebytes(bytearray(base64string, encoding='utf-8')))

		res = analyze_pdf('test.pdf')
